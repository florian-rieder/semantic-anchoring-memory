...
class ABox():
    ...
    def query_entities(self, query: str, k=4):
        ...
        self.entities_db.similarity_search()
        return [d.page_content for d in self.entities_db.similarity_search_with_relevance_scores(query, k)]

    def query_entities_with_score(self, query: str, threshold: float = 0.75, k=4):
        """Get the entities which are most relevant to the query"""
        # Query with score:
